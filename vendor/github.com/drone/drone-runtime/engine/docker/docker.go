...
func (e *dockerEngine) Destroy(ctx context.Context, spec *engine.Spec) error {
	...
	for _, step := range spec.Steps {
		// TODO: Check for status of step
		if step.Docker != nil && step.Docker.StopTimeout != "" {
			n, _ := strconv.Atoi(step.Docker.StopTimeout)
			var t *time.Duration
			if n > 0 {
				dur := time.Duration(n) * time.Second
				t = &dur
			}
			e.client.ContainerStop(ctx, step.Metadata.UID, t)
		} else {
